...
public class PrimeApplicationTests {
    ...
    @Test
    @DisplayName("Test de búsqueda de primos")
    void testPrimesSearch() {
        // Test de generación de gráfica
        System.out.println("\n=== TEST DE GENERACIÓN DE GRÁFICA ===");
        String chartPath = Util.chart(1000);
        if (chartPath != null) {
            System.out.println("✅ Gráfica generada exitosamente en: " + chartPath);
        } else {
            System.out.println("❌ Error al generar la gráfica");
        }
    }
    ...
    private void printResult(int number, PerformanceResult result) {
        String status = result.averageTimeMs < 100 ? "✅" : result.averageTimeMs < 500 ? "⚠️" : "🚨";
